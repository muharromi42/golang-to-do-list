...
func main() {
    ...
    app.Put("/activities/:id", func (c *fiber.Ctx) error  {
        ...
        sqlStatement := `UPDATE activities SET title = $1, category = $2, description = $3, activity_date = $4 WHERE id = $5
        RETURNING id`
        ...
        if err != nil{
            return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"status": err.Error()})
        }
